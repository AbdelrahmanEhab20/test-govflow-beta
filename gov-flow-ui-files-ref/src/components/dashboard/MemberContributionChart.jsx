            ...
            <Tooltip 
              content={({ active, payload }) => {
                if (active && payload?.length) {
                  ...
                  return (
                    <div className="bg-white dark:bg-slate-900 p-3 border border-slate-200 dark:border-slate-700 rounded shadow-lg">
                      <p className="font-medium text-slate-900 dark:text-slate-100">{data.fullName}</p>
                      <p className="text-sm text-slate-600 dark:text-slate-300">Led: {data.led}</p>
                      <p className="text-sm text-slate-600 dark:text-slate-300">Supported: {data.supported}</p>
                      <p className="text-sm text-slate-600 dark:text-slate-300">Completed: {data.completed}</p>
                    </div>
                  );
                }
                return null;
