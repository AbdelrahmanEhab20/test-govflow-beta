...
# Production: Cloudflare R2 (recommended on Render free tier — local disk is ephemeral)
STORAGE_PROVIDER=r2
S3_BUCKET=govflow-avatars-beta
S3_REGION=auto
S3_ENDPOINT=https://<account_id>.r2.cloudflarestorage.com
S3_ACCESS_KEY_ID=<r2-access-key>
S3_SECRET_ACCESS_KEY=<r2-secret-key>
S3_PUBLIC_BASE_URL=https://pub-xxxxxxxx.r2.dev
UPLOADS_MAX_SIZE=5242880  # 5MB for avatars

# Local dev fallback (when S3_* vars are unset)
UPLOADS_DIR=./uploads
...
| Avatar images show initials / 404 on `/uploads/*` | Render ephemeral disk wiped files; MongoDB still has old URLs | Use Cloudflare R2 (`STORAGE_PROVIDER=r2` + `S3_*` vars); users re-upload avatars from Profile |
| Permission denied on uploads | Incorrect directory permissions (local dev only) | `chmod 755 ./uploads` |
