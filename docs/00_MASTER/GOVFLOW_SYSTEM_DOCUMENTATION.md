...
- **Production (recommended):** Avatars upload via `POST /auth/avatar` → stored in **Cloudflare R2** (S3-compatible). MongoDB `avatar_url` holds the public R2 URL (e.g. `https://pub-xxx.r2.dev/avatars/...`). Configure `STORAGE_PROVIDER=r2` plus `S3_*` env vars on the backend.
- **Local dev fallback:** If R2 env vars are not set, files are stored under `UPLOADS_DIR` and served at **`GET /uploads/<filename>`**.
- **Render free tier:** Local `./uploads` is ephemeral — files are lost on redeploy. Do not rely on `/uploads` URLs in production on Render without persistent disk or object storage.
